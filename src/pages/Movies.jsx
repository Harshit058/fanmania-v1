...
const Movies = () => {
  ...
  const getCastDetails = async (tmdbId) => {
    try {
      const response = await axios.get(`https://api.themoviedb.org/3/movie/${tmdbId}/credits?api_key=${TMDB_API_KEY}`);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const tmdbResponse = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(userInput)}`);
      ...
      const movieCards = await Promise.all(movies.map(async (movie) => {
        try {
          ...
          if (tmdbMovie) {
            ...
            const tmdbSearchResponse = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(detailedMovie.Title)}`);
